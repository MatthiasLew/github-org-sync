...
def test_sync_repositories_clone_and_skip(sync_service: SyncService, mock_git_service: MagicMock) -> None:
    ...
    mock_git_service.clone.return_value = SyncResult(
        repo_name="repo1", status="CLONED", before_status="MISSING", after_status="UP_TO_DATE", duration=1.0, operation="clone", message="Cloned"
    )
    mock_git_service.sync.return_value = SyncResult(
        repo_name="repo3", status="UP_TO_DATE", before_status="UP_TO_DATE", after_status="UP_TO_DATE", duration=0.5, operation="sync", message="Sync"
    )
...
def test_sync_repositories_cancellation(sync_service: SyncService, mock_git_service: MagicMock) -> None:
    ...
    workspace = Path("/dummy/workspace")
    results = sync_service.sync_repositories(repos, workspace, "org", {"use_ssh": False}, is_cancelled_callback=is_cancelled)
