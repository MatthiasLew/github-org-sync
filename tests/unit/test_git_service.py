...
def test_sync_success_clean(mock_run: MagicMock, mock_get_status: MagicMock, git_service: GitService) -> None:
    repo = Repository(name="myrepo", url="https://github.com/org/myrepo", ssh_url="git@github.com:org/myrepo.git", local_path=Path("/dummy/myrepo"))
    ...
    with patch("pathlib.Path.exists") as mock_exists:
        mock_exists.return_value = True
        res = git_service.sync(repo, "org", preserve_local_changes=True, fetch_only=False, checkout_default=False, dry_run=False)
