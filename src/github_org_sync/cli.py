...
def main(argv: List[str] | None = None) -> int:
    ...
    filter_parser.add_argument("--include-forks", action="store_true", default=True, help="Include forks (default: True)")
    filter_parser.add_argument("--exclude-forks", dest="include_forks", action="store_false", help="Exclude forks")
    
    # 1. 'list' command
    list_parser = subparsers.add_parser("list", parents=[org_parser, filter_parser], help="List organization repositories on GitHub")
    
    # 2. 'status' command
    status_parser = subparsers.add_parser("status", parents=[org_parser, workspace_parser, filter_parser], help="Check status of local repositories")
    
    # 3. 'sync' command
    sync_parser = subparsers.add_parser("sync", parents=[org_parser, workspace_parser, filter_parser], help="Synchronize (clone & update) organization repositories")
    ...
    try:
        ...
        if args.command == "sync":
            ...
            def progress_chk(curr, tot, name):
            ...
            # Sync
            def progress_sync(curr, tot, repo, res):
