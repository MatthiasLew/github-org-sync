...
### Added
- **Visual History Log Graph (`git log --graph`)**: Dedicated "Git History Graph" tab and context dialog (`LogGraphDialog`) displaying interactive visual commit trees with ASCII branching graphs, branch/tag decorations, customizable commit limit (25-200), and all-branches toggle (`--all`). Accessible from MainWindow Tab 3 and right-click context menu on repositories.
...
### Added
- **Git Stash Drawer / Visual Manager**: Visual drawer and management dialog for repository stash stack (`git stash list`). Supports inspecting stash diffs/stats, popping stashes (`stash pop`), dropping entries (`stash drop`), and creating new stashes (`stash push`) with optional messages.

## [1.5.0] - 2026-09-04

### Added
- **GUI Commit & Staging Manager**: Stage/unstage individual or all modified files and commit changes directly from the GUI with custom commit messages. Accessible via the right-click context menu on DIRTY repositories and inside the ResolveIssueDialog.

## [1.4.0] - 2026-08-03

### Added
- **Launch Git Merge Tool**: Async launch of system merge/diff tool (`git mergetool`) from the conflict resolution dialog.
- **Git Branch Switcher**: Right-click context menu dialog listing local branches to checkout.
- **Hosting Provider Badges**: Color-coded "Host" column for GitHub (blue), GitLab (orange), and Bitbucket (sky blue) references.
- **Offline Workspace State Cache**: Cache workspace repo states locally for instant offline GUI startup without disk scans.

## [1.3.10] - 2026-08-03

### Added
- **Diagnostic Checklist Panel**: Diagnostic checklist panel under Help -> Run Diagnostics checking Git, gh CLI auth, and SSH connection.

## [1.3.9] - 2026-08-03

### Changed
- **GUI Tabbed Redesign**: Redesigned MainWindow layout with tabbed cards and a dedicated full-height logs tab.

## [1.3.8] - 2026-08-03

### Added
- **Persistent Rotating Log Files**: Save standard logging to `%APPDATA%/github-org-sync/logs/app.log` (Windows) / `~/.config/github-org-sync/logs/app.log` (Unix) with rotating backups.

## [1.3.7] - 2026-08-03

### Added
- **"Open in Terminal" Row Action**: Row context action to open native command prompt/shell inside local repository directory.

## [1.3.5] - 2026-07-28

### Added
- **Parallel Repository Syncing**: Multi-threaded checkout, clone, and local git status inspection using ThreadPoolExecutor.

## [1.3.2] - 2026-07-25

### Added
- **Automatic Updates**: Added automatic update checker on startup and manual check option in the Help menu. Downloads and installs updates directly from GitHub Releases.
- Platform-specific update replacement script runners (Windows bat file, Unix inline unlinking & process re-execution).
